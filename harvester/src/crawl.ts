...
export async function crawlReturned({
  ACCESS_TOKEN,
  SEARCH_KEYWORDS,
  TWEET_THREAD_URL,
  SEARCH_FROM_DATE,
  ...
  ACCESS_TOKEN: string;
  SEARCH_KEYWORDS?: string;
  SEARCH_FROM_DATE?: string | Date;
  SEARCH_TO_DATE?: string | Date;
  TARGET_TWEET_COUNT?: number;
  DELAY_EACH_TWEET_SECONDS?: number;
  DELAY_EVERY_100_TWEETS_SECONDS?: number;
  DEBUG_MODE?: boolean;
  ...
  async function startCrawlTwitter({
    ...
    const allData: { tweets: TweetMappedReturn[] } = {
      tweets: [],
    };
    ...
    return allData.tweets;
  }

  const accumulator: TweetMappedReturn[] = [];
  try {
